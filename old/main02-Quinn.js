...
function showTable(tableHTML) {
  ...
  var codeStyleHTML = formatHTML(tableHTML);
  console.log(codeStyleHTML)
...
function formatHTML(parsedStack) {
  ...
  for (var i=0; i < parsedStack.length; i++) {
    ...
    switch(start) {
      case '<tr':
        parsedStack[i]="&lt;\'"+parsedStack[i].splice(1);
        console.log(parsedStack[i])
        // showStack[i] = "'" + grammarStack[i];
        for (spaces = 0; spaces < indentLevel*indentWidth; spaces++) {
          parsedStack[i] = " "+showStack[i]
        };
        indentLevel++;
        showStack[i] += '<br>';
        break;
